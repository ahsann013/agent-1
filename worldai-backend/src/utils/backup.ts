...
import { decodeStream } from '../utils/reader.js';
...
export class AppService {
  ...
  setupTools() {
    ...
    const image_to_image = tool(
      async ({ prompt, image_url, strength = 0.75, width, height, steps, userId = null }) => {
    ...
    const enhance_image = tool(
      async ({ image_url, enhancement_type = "quality", width, height }) => {
...
`
  ...
  async runAgent(message, fileInfo, userSettings, userId, chatId) {
    ...
    // Mandatory prompt with tool instructions and settings
