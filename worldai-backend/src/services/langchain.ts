...
export class AppService {
  ...
  setupTools() {
    ...
    const deduct_credits = tool(
      async ({ toolName, params = {}, tokenInfo = null }) => {
        console.log('Deducting credits for tool:', toolName);
        try {
          // Skip credit deduction for generate_completions
          ...
          // Handle token-based credit calculation for text/code generation
          ...
          // Handle video duration-based pricing
          ...
          // Handle per-run or per-image pricing
          else {
    ...
    const generate_completions = tool(
      async ({ prompt }) => {
        if (!this.isInitialized) {
        ...
        try {
          ...
          // Handle array output by joining strings with newlines
    ...
    const generate_image = tool(
      async ({ prompt, width, height, steps, userId = null }) => {
    ...
    const generate_video = tool(
      async ({ prompt, num_frames, fps, width, height, userId = null }) => {
    ...
    const image_to_image = tool(
      async ({ prompt, image_url }) => {
        ...
        try {
          ...
          // First, generate a detailed product photography description
    ...
    const generate_music = tool(
      async ({ prompt, duration = 8, temperature = 1, model_version = "stereo-large" }) => {
      ...
      async ({ image_url }) => {
        ...
        try {
          ...
          const output = await replicate.run(
            "ndreca/hunyuan3d-2:4ac0c7d1ef7e7dd58bf92364262597272dea79bfdb158b26027f54eb667f28b8",
            {
              input: {
                seed: 1234,
                image: image_url,
          ...
          // Check if we have a valid output with mesh URL
    ...
    const voice_cloner = tool(
      async ({ text, speaker_url }) => {
    ...
    const generate_code = tool(
      async ({ prompt, language = "javascript" }) => {
    ...
    const product_photography = tool(
      async ({ prompt, image_url }) => {
        console.log('Generate image with Gemini tool called with prompt:', prompt);
        const mdl = await ApiSetting.findOne({
          where: {
    ...
    const image_to_video = tool(
      async ({ image_url, prompt, motion_scale = 1, num_frames = 50, fps = 6, userId = null }) => {
  ...
  async runAgent(message, fileInfo, userSettings, userId, chatId) {
    ...
    if (chatId) {
      try {
        ...
        // Transform messages into the format expected by the LLM
    ...
    // Base system prompt from database is mandatory
...
Provide the JSON object directly without any backticks or formatting. Response format: { message: <string>, imageUrl: <string>, videoUrl: <string>, audioUrl: <string>, modelUrl: <string>, code: <string> }`;
    ...
    // Combine with file info if present
    ...
    try {
      ...
      // Try to parse the message if it's already JSON
      let responseObject;
      try {
        responseObject = typeof finalMessage === 'string' ? JSON.parse(finalMessage) : finalMessage;
      } catch (parseError) {
        // If parsing fails, create a properly formatted response
        responseObject = {
      ...
      // Calculate total token usage across all tool calls
      ...
      // Ensure all required fields exist and add usage statistics
      responseObject = {
      ...
      // Create a graceful error response maintaining the required format
      const errorResponse = {
...
If message is "How do I implement authentication in Node.js?" -> "Node.js Authentication Implementation"`;
  ...
  async inPainting(imageBuffer, prompt, userId = null, maskBuffer = null) {
    ...
    try {
      ...
      if (userId) {
        // Get service pricing for inpainting
        const servicePricing = await ServicePricing.findOne({
          where: {
